...
mod tests {
    ...
    fn parse(source: &str) -> korlix_ast::program::Module {
        ...
        assert!(!lex_diag.has_errors(), "Lex errors: {:?}", lex_diag);
        let parser = Parser::new(&tokens, 0);
        let (module, parse_diag) = parser.parse(PathBuf::from("<test>"));
        assert!(!parse_diag.has_errors(), "Parse errors: {:?}", parse_diag);
        module
    }
...
"#,
        ...
        if let korlix_ast::program::Item::Page(page) = &module.items[0] {
            let has_query = page
                .body
                .iter()
                .any(|n| matches!(n, Node::ApiQuery(q) if q.name == "users"));
            assert!(
                has_query,
                "Expected ApiQuery(users) in page body, got: {:?}",
                page.body
            );
        } else {
...
"#,
        ...
        if let korlix_ast::program::Item::Page(page) = &module.items[0] {
            ...
            let has_post = a
                .body
                .iter()
                .any(|n| matches!(n, Node::ApiMutation(m) if m.url == "/api/users"));
            ...
            let has_reload = a
                .body
                .iter()
                .any(|n| matches!(n, Node::ApiReload(r) if r.target == "users"));
