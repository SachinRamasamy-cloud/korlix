...
mod tests {
    ...
    fn test_lex_keywords() {
        let (tokens, diag) = lex(r#"page index route "/":"#, 0);
    ...
    fn test_lex_class() {
        ...
        let classes: Vec<_> = tokens
            .iter()
            .filter_map(|t| {
                if let TokenKind::Class(c) = &t.kind {
                    Some(c.as_str())
                } else {
                    None
                }
    ...
    fn test_lex_on_event() {
        ...
        assert!(tokens
            .iter()
            .any(|t| matches!(&t.kind, TokenKind::OnEvent(e) if e == "click")));
    }

    #[test]
    fn test_lex_indentation() {
        let src = r#"page index:
  h1 "Hello"
  p "World""#;
...
h1 "Hello""#,
        ...
        assert!(tokens
            .iter()
            .any(|t| matches!(&t.kind, TokenKind::Ident(s) if s == "h1")));
    }
    ...
    fn test_lex_arbitrary_class() {
        ...
        assert!(tokens
            .iter()
            .any(|t| matches!(&t.kind, TokenKind::Class(c) if c == "w-[320px]")));
    }

    #[test]
    fn test_lex_complex_arbitrary_class() {
        let (tokens, _) = lex("div .bg-[rgba(10,8,25,0.85)] .bg-[linear-gradient(135deg,#7c3aed,#059669)] .hover:bg-[rgba(124,58,237,0.1)]", 0);
        let classes: Vec<_> = tokens
            .iter()
            .filter_map(|t| {
                if let TokenKind::Class(c) = &t.kind {
