...
/// Returns the full Korlix color palette as (name → hex).
pub fn build_palette() -> HashMap<String, String> {
...
/// CSS variable declarations for all colors.
pub fn build_css_vars(palette: &HashMap<String, String>) -> String {
    let mut vars = String::from(":root {\n");
    let mut keys: Vec<_> = palette.keys().collect();
    keys.sort();
    for key in keys {
        vars.push_str(&format!("  --kx-{}: {};\n", key, palette[key]));
    }
    vars.push_str("}\n");
    vars
}
