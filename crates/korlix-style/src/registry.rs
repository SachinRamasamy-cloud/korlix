...
impl CssRule {
    pub fn new(selector: impl Into<String>) -> Self {
        Self { selector: selector.into(), declarations: vec![], media_query: None }
    }
...
fn build_registry() -> IndexMap<String, CssRule> {
    ...
    for cols in 1..=12 {
        ...
        add!(cls.clone(), CssRule::new(&cls).prop("grid-template-columns", format!("repeat({},minmax(0,1fr))", cols)));
        let cls2 = format!("col-span-{}", cols);
        add!(cls2.clone(), CssRule::new(&cls2).prop("grid-column", format!("span {}/span {}", cols, cols)));
    }
    add!("grid-cols-none", CssRule::new("grid-cols-none").prop("grid-template-columns","none"));
    for rows in 1..=6 {
        let cls = format!("grid-rows-{}", rows);
        add!(cls.clone(), CssRule::new(&cls).prop("grid-template-rows", format!("repeat({},minmax(0,1fr))", rows)));
    }
    ...
    for (scale, px) in SPACING {
        let v = px.to_string();
        // padding all sides
        add!(format!("p-{}", scale),  CssRule::new(format!("p-{}", scale)).prop("padding", &v));
        add!(format!("px-{}", scale), CssRule::new(format!("px-{}", scale)).prop("padding-left",&v).prop("padding-right",&v));
        add!(format!("py-{}", scale), CssRule::new(format!("py-{}", scale)).prop("padding-top",&v).prop("padding-bottom",&v));
        add!(format!("pt-{}", scale), CssRule::new(format!("pt-{}", scale)).prop("padding-top",&v));
        add!(format!("pr-{}", scale), CssRule::new(format!("pr-{}", scale)).prop("padding-right",&v));
        add!(format!("pb-{}", scale), CssRule::new(format!("pb-{}", scale)).prop("padding-bottom",&v));
        add!(format!("pl-{}", scale), CssRule::new(format!("pl-{}", scale)).prop("padding-left",&v));
        // margin
        add!(format!("m-{}", scale),  CssRule::new(format!("m-{}", scale)).prop("margin",&v));
        add!(format!("mx-{}", scale), CssRule::new(format!("mx-{}", scale)).prop("margin-left",&v).prop("margin-right",&v));
        add!(format!("my-{}", scale), CssRule::new(format!("my-{}", scale)).prop("margin-top",&v).prop("margin-bottom",&v));
        add!(format!("mt-{}", scale), CssRule::new(format!("mt-{}", scale)).prop("margin-top",&v));
        add!(format!("mr-{}", scale), CssRule::new(format!("mr-{}", scale)).prop("margin-right",&v));
        add!(format!("mb-{}", scale), CssRule::new(format!("mb-{}", scale)).prop("margin-bottom",&v));
        add!(format!("ml-{}", scale), CssRule::new(format!("ml-{}", scale)).prop("margin-left",&v));
        // gap
        add!(format!("gap-{}", scale),   CssRule::new(format!("gap-{}", scale)).prop("gap",&v));
        add!(format!("gap-x-{}", scale), CssRule::new(format!("gap-x-{}", scale)).prop("column-gap",&v));
        add!(format!("gap-y-{}", scale), CssRule::new(format!("gap-y-{}", scale)).prop("row-gap",&v));
    ...
    add!("mx-auto", CssRule::new("mx-auto").prop("margin-left","auto").prop("margin-right","auto"));
    add!("my-auto", CssRule::new("my-auto").prop("margin-top","auto").prop("margin-bottom","auto"));

    // ── Sizing ───────────────────────────────────────────────────────
    for (scale, px) in SPACING {
        let v = px.to_string();
        add!(format!("w-{}", scale), CssRule::new(format!("w-{}", scale)).prop("width",&v));
        add!(format!("h-{}", scale), CssRule::new(format!("h-{}", scale)).prop("height",&v));
        add!(format!("min-w-{}", scale), CssRule::new(format!("min-w-{}", scale)).prop("min-width",&v));
        add!(format!("min-h-{}", scale), CssRule::new(format!("min-h-{}", scale)).prop("min-height",&v));
        add!(format!("max-w-{}", scale), CssRule::new(format!("max-w-{}", scale)).prop("max-width",&v));
        add!(format!("max-h-{}", scale), CssRule::new(format!("max-h-{}", scale)).prop("max-height",&v));
        add!(format!("size-{}", scale), CssRule::new(format!("size-{}", scale)).prop("width",&v).prop("height",&v));
    }
    for (cls,prop,val) in [
        ...
        ("max-w-full","max-width","100%"),("max-w-screen","max-width","100vw"),
    ] {
        add!(cls, CssRule::new(cls).prop(prop,val));
    }
    ...
    for (name, fs, lh) in FONT_SIZES {
        ...
        add!(cls.clone(), CssRule::new(&cls).prop("font-size",*fs).prop("line-height",*lh));
    }
    ...
    for (color_name, hex) in &palette {
        ...
        for prefix in ["text","bg","border","ring","fill","stroke","outline","caret","placeholder"] {
    ...
    for (name, val) in RADIUS {
        let cls = if name.is_empty() { "rounded".to_string() } else { format!("rounded-{}", name) };
        add!(cls.clone(), CssRule::new(&cls).prop("border-radius",*val));
    }
    for w in [0,1,2,4,8] {
        let cls = if w == 1 { "border".to_string() } else { format!("border-{}", w) };
        let v = if w == 0 { "0px".to_string() } else { format!("{}px", w) };
    ...
    for (name, val) in SHADOWS {
        let cls = if name.is_empty() { "shadow".to_string() } else { format!("shadow-{}", name) };
    ...
    // ── Overflow ─────────────────────────────────────────────────────
    for (cls,val) in [
        ("overflow-auto","auto"),("overflow-hidden","hidden"),
        ("overflow-visible","visible"),("overflow-scroll","scroll"),
        ("overflow-x-auto","auto"),("overflow-y-auto","auto"),
        ("overflow-x-hidden","hidden"),("overflow-y-hidden","hidden"),
    ] {
        let prop = if cls.contains("-x-") { "overflow-x" } else if cls.contains("-y-") { "overflow-y" } else { "overflow" };
        add!(cls, CssRule::new(cls).prop(prop, val));
    }
    ...
    for (name, val) in TRANSITIONS {
        let cls = if name.is_empty() { "transition".to_string() } else { format!("transition-{}", name) };
        add!(cls.clone(), CssRule::new(&cls).prop("transition",*val));
    }
    for (name, val) in DURATIONS {
        let cls = format!("duration-{}", name);
        add!(cls.clone(), CssRule::new(&cls).prop("transition-duration",*val));
    }

    // ── Cursor / Pointer ─────────────────────────────────────────────
    for (cls, val) in [
        ("cursor-auto","auto"),("cursor-default","default"),("cursor-pointer","pointer"),
        ("cursor-wait","wait"),("cursor-text","text"),("cursor-not-allowed","not-allowed"),
        ("cursor-grab","grab"),("cursor-grabbing","grabbing"),
        ("pointer-events-none","none"),("pointer-events-auto","auto"),
        ("select-none","none"),("select-text","text"),("select-all","all"),
    ] {
        let prop = if cls.starts_with("pointer") { "pointer-events" }
                   else if cls.starts_with("select") { "user-select" }
                   else { "cursor" };
        add!(cls, CssRule::new(cls).prop(prop,val));
    }
    ...
    // ── Accessibility ────────────────────────────────────────────────
    add!("sr-only", CssRule::new("sr-only")
        .prop("position","absolute").prop("width","1px").prop("height","1px")
        .prop("padding","0").prop("margin","-1px").prop("overflow","hidden")
        .prop("clip","rect(0,0,0,0)").prop("white-space","nowrap")
        .prop("border-width","0"));
    add!("not-sr-only", CssRule::new("not-sr-only")
        .prop("position","static").prop("width","auto").prop("height","auto")
        .prop("padding","0").prop("margin","0").prop("overflow","visible")
        .prop("clip","auto").prop("white-space","normal"));

    // ── Misc ──────────────────────────────────────────────────────────
    for (cls,prop,val) in [
        ("outline-none","outline","2px solid transparent"),
        ("ring","box-shadow","0 0 0 3px var(--kx-ring-color,rgba(99,102,241,.5))"),
        ("ring-2","box-shadow","0 0 0 2px var(--kx-ring-color,rgba(99,102,241,.5))"),
        ...
        ("blur-none","filter","blur(0)"),("blur-sm","filter","blur(4px)"),
        ("blur","filter","blur(8px)"),("blur-md","filter","blur(12px)"),
    ...
    for (scale, px) in SPACING {
        for dir in ["inset","top","right","bottom","left"] {
            let cls = if dir == "inset" { format!("inset-{}", scale) } else { format!("{}-{}", dir, scale) };
            let prop = if dir == "inset" { "inset".to_string() } else { dir.to_string() };
            add!(cls.clone(), CssRule::new(&cls).prop(prop, *px));
        }
    }
    for (cls, prop, val) in [
        ("inset-x-0","left","0"),("inset-y-0","top","0"),
        ("inset-full","inset","100%"),("inset-auto","inset","auto"),
    ] {
        add!(cls, CssRule::new(cls).prop(prop,val));
    }
...
pub fn suggest(class: &str) -> Vec<String> {
    ...
    let mut scored: Vec<(usize, &String)> = names
        ...
        .filter_map(|n| {
            ...
            if dist <= 3 { Some((dist, n)) } else { None }
        })
        .collect();
