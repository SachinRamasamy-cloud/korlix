...
pub fn analyze_features(module: &Module) -> HashSet<RuntimeFeature> {
    ...
    for item in &module.items {
        match item {
            Item::AppDecl(app) => {
                if !app.routes.is_empty() { features.insert(RuntimeFeature::Router); }
                for p in &app.providers {
                    match p.as_str() {
                        "toast"  => { features.insert(RuntimeFeature::Toast); }
                        "modal"  => { features.insert(RuntimeFeature::Overlay); }
                        "theme"  => { features.insert(RuntimeFeature::Theme); }
                        _ => {}
                    }
                }
            }
    ...
    for node in nodes {
        match node {
            Node::Component(c) => {
                match c.name.as_str() {
                    "toast"   => { features.insert(RuntimeFeature::Toast); }
                    "modal" | "drawer" | "tooltip" => { features.insert(RuntimeFeature::Overlay); }
                    "image" | "avatar" => { features.insert(RuntimeFeature::Media); }
            ...
            Node::Element(el) => {
                for _ev in &el.events { features.insert(RuntimeFeature::Core); }
                scan_nodes_features(&el.children, features);
            }
            Node::State(_) | Node::Derived(_) => { features.insert(RuntimeFeature::State); }
            Node::If(i) => {
                scan_nodes_features(&i.then_body, features);
                if let Some(e) = &i.else_body { scan_nodes_features(e, features); }
            }
