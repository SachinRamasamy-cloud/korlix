...
pub fn expand_component(node: &ComponentNode) -> Node {
    ...
    let mut classes: Vec<ClassRef> = schema
        .default_classes
        .iter()
...
fn expand_button(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    if loading {
        children.insert(
            ...
            Node::Element(ElementNode {
                tag: "span".into(),
                classes: vec![ClassRef::new("kx-spinner kx-spinner--sm", node.span)],
                props: vec![],
                events: vec![],
                children: vec![],
                span: node.span,
    ...
    let mut props = vec![Prop::new(
        "type",
        Expr::String(btn_type.to_string()),
        node.span,
    )];
    if disabled {
...
fn expand_icon(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    Node::Element(ElementNode {
        ...
        props,
        events: node.events.clone(),
        children: vec![Node::Raw(korlix_ast::node::RawNode {
            html: svg_content,
            span: node.span,
        })],
        span: node.span,
...
fn expand_skeleton(node: &ComponentNode, mut classes: Vec<ClassRef>) -> Node {
    ...
    Node::Element(ElementNode {
        ...
        props: vec![
            Prop::new(
                "style",
                Expr::String(format!("width:{};height:{}", w, h)),
                node.span,
            ),
    ...
    for _ in 0..count.min(10) {
        children.push(Node::Element(ElementNode {
            ...
            children: vec![
                Node::Element(ElementNode {
                    tag: "div".into(),
                    classes: vec![ClassRef::new("kx-skeleton", node.span)],
                    props: vec![Prop::new(
                        "style",
                        Expr::String("width:100%;height:180px;margin-bottom:.75rem".into()),
                        node.span,
                    )],
                    events: vec![],
                    children: vec![],
                ...
                Node::Element(ElementNode {
                    ...
                    props: vec![Prop::new(
                        "style",
                        Expr::String("width:70%;height:1rem;margin-bottom:.5rem".into()),
                        node.span,
                    )],
                    events: vec![],
                    children: vec![],
                ...
                Node::Element(ElementNode {
                    ...
                    props: vec![Prop::new(
                        "style",
                        Expr::String("width:90%;height:.75rem".into()),
                        node.span,
                    )],
                    events: vec![],
                    children: vec![],
...
fn expand_empty_state(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    if let Some(ic) = icon {
        children.push(Node::Element(ElementNode {
            tag: "div".into(),
            classes: vec![ClassRef::new("kx-empty-state__icon", node.span)],
            props: vec![Prop::new(
                "data-icon",
                Expr::String(ic.to_string()),
                node.span,
            )],
            events: vec![],
            children: vec![],
    ...
    children.push(Node::Element(ElementNode {
        tag: "h3".into(),
        classes: vec![ClassRef::new("kx-empty-state__title", node.span)],
        props: vec![],
        events: vec![],
        children: vec![Node::Text(korlix_ast::node::TextNode {
            value: Expr::String(title.to_string()),
            span: node.span,
        })],
        span: node.span,
    }));
    if let Some(d) = desc {
        children.push(Node::Element(ElementNode {
            tag: "p".into(),
            classes: vec![ClassRef::new("kx-empty-state__desc", node.span)],
            props: vec![],
            events: vec![],
            children: vec![Node::Text(korlix_ast::node::TextNode {
                value: Expr::String(d.to_string()),
                span: node.span,
            })],
            span: node.span,
        }));
    ...
    Node::Element(ElementNode {
        tag: "div".into(),
        classes,
        props: vec![Prop::new("role", Expr::String("status".into()), node.span)],
        events: vec![],
        children,
        span: node.span,
    })
...
fn expand_toast_trigger(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    let msg = get_prop_str(node, "message")
        .or_else(|| {
            node.children.first().and_then(|c| {
                if let Node::Text(t) = c {
                    t.value.as_string().map(|s| s)
                } else {
                    None
                }
            })
...
fn expand_modal(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    if let Some(t) = title {
        children.push(Node::Element(ElementNode {
            tag: "div".into(),
            classes: vec![ClassRef::new("kx-modal-header", node.span)],
            props: vec![],
            events: vec![],
            children: vec![
                Node::Element(ElementNode {
...
fn expand_progress(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    Node::Element(ElementNode {
        ...
        props: vec![
            ...
            Prop::new(
                "style",
                Expr::String(format!("--kx-progress:{:.2}%", pct)),
                node.span,
            ),
        ],
        events: node.events.clone(),
...
fn expand_link(node: &ComponentNode, classes: Vec<ClassRef>) -> Node {
    ...
    if external {
        props.push(Prop::new(
            "target",
            Expr::String("_blank".into()),
            node.span,
        ));
        props.push(Prop::new(
            "rel",
            Expr::String("noopener noreferrer".into()),
            node.span,
        ));
    } else {
        props.push(Prop::new(
