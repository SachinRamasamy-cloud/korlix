...
pub fn run(a11y: bool, security: bool, seo: bool, ast: bool) -> anyhow::Result<()> {
    ...
    println!("  {} Checking {} files…", "◈".cyan().bold(), klx_files.len());
    ...
    for path in &klx_files {
        ...
        if diag.has_errors() || diag.warning_count() > 0 {
            ...
            for d in &diag.diagnostics {
                ...
                if let Some(h) = &d.hint { println!("      {} {}", "hint:".green(), h); }
            }
        }
    ...
    println!();
    if total_errors > 0 {
        println!("  {} {} error(s), {} warning(s)", "✕".red().bold(), total_errors, total_warnings);
        std::process::exit(1);
    } else if total_warnings > 0 {
        println!("  {} {} warning(s) found", "⚠".yellow().bold(), total_warnings);
    } else {
