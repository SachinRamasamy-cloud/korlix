use korlix_ast::{expression::Expr, node::Node, element::ElementNode};
...
pub fn render_node(node: &Node) -> String {
    match node {
        ...
        Node::Slot(s)        => format!(r#"<div data-slot="{}" class="kx-slot"></div>"#, s.name.as_deref().unwrap_or("default")),
...
fn render_element(el: &ElementNode) -> String {
    ...
    for prop in &el.props {
        ...
        if key.starts_with("on") && val.to_lowercase().contains("javascript:") { continue; }
        attrs.push_str(&format!(r#" {}="{}""#, html_attr_key(key), html_escape_attr(&val)));
    }

    // Encode events as data attributes for runtime
    for ev in &el.events {
        let body_js = nodes_to_js_stub(&ev.body);
        attrs.push_str(&format!(r#" data-on-{}="{}""#, ev.event, html_escape_attr(&body_js)));
    }
...
fn render_if_static(i: &korlix_ast::node::IfNode) -> String {
    ...
    let else_html = i.else_body.as_ref().map(|e| render_nodes(e)).unwrap_or_default();
    let cond = render_expr(&i.condition);
    format!(
        r#"<template data-kx-if="{}">{}</template><template data-kx-else="{}">{}</template>"#,
        html_escape_attr(&cond), then_html, html_escape_attr(&cond), else_html
    )
}
...
fn render_expr(e: &Expr) -> String {
    match e {
        ...
        Expr::Member { object, field } => {
            format!(r#"<span data-kx-bind="{}.{}"></span>"#, render_expr_raw(object), field)
        }
        Expr::Call { callee, args } => {
            let a = args.iter().map(render_expr_raw).collect::<Vec<_>>().join(",");
            format!(r#"<span data-kx-expr="{}({})"></span>"#, render_expr_raw(callee), a)
        }
...
pub fn render_expr_raw(e: &Expr) -> String {
    match e {
        ...
        Expr::Binary { left, op, right } => {
            ...
            format!("({} {} {})", render_expr_raw(left), op_str, render_expr_raw(right))
        }
        Expr::Call { callee, args } => {
            let a = args.iter().map(render_expr_raw).collect::<Vec<_>>().join(", ");
...
fn nodes_to_js_stub(nodes: &[Node]) -> String {
    nodes.iter().map(|n| match n {
        ...
        Node::Call(c)   => format!(
            "KorlixRuntime.call('{}', [{}])",
            c.callee,
            c.args.iter().map(render_expr_raw).collect::<Vec<_>>().join(",")
        ),
        Node::Component(c) if c.name == "toast" => {
            let args = c.children.iter().filter_map(|child| {
...
fn render_expr_state(e: &Expr) -> String {
    match e {
        ...
        Expr::Binary { left, op, right } => {
            ...
            format!("({} {} {})", render_expr_state(left), op_str, render_expr_state(right))
        }
        Expr::Call { callee, args } => {
            let a = args.iter().map(render_expr_state).collect::<Vec<_>>().join(", ");
